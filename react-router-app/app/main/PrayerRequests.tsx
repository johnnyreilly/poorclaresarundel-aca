...
import { prayerRequestsApiPath } from "./routePaths";
...
export function PrayerRequests() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const rawResponse = await fetch(prayerRequestsApiPath, {
        method: "POST",
        headers: {
      ...
      {message && (
        <Alert color={message.ok ? "success" : "danger"}>
          <img
            ...
            src={smalltau}
            alt={message.ok ? "success" : "failure"}
          />
          {message.text}
        </Alert>
      )}
      ...
      <form name="form" noValidate onSubmit={handleSubmit}>
        <div
          className={`form-group has-feedback ${
            formSubmitAttempted && !requestEmail ? "has-error" : ""
          }`}
        >
          ...
          <input
            ...
            required
          />
          {formSubmitAttempted && !requestEmail && (
        ...
        <div>
          ...
          <textarea
            ...
            required
          />
          {formSubmitAttempted && !requestPrayFor && (
            <span>You must tell us something to pray for!</span>
          )}
        </div>
        <button
          disabled={!requestEmail || !requestPrayFor}
          type="submit"
